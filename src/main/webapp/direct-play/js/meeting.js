    ...
    $(function() {
        ...
        (function initAgoraRTC() {
            client.init(key, function(obj) {
                ...
                //     client.setScreenSharingWindow('', function(){alert('success')}, function(){alert('failure')})
                // }, function(){
                //     alert('error')
                // })


                client.getWindows(function(windows){
                    for (var i = 0; i < windows.length; ++i) {
                        windows[i].title = atob(windows[i].title)
                        if (windows[i].title.indexOf('123.txt')) {
                            client.setScreenSharingWindow(windows[i].windowId, function(){alert('success')}, function(){alert('failure')})
                            break
                        }
                    }
                    //alert(JSON.stringify(windows))
                    //client.startScreenSharing(windows[0].windowId)
                })
